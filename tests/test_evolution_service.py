    ...
    return [
        {
            ...
            "name": "Parent A",
            "model": "gpt-4",
            "personality_prompt": {"role": "ParentA"},
            "enabled": True,
        },
        {
            "id": "p2",
            "name": "Parent B",
            "model": "gpt-4",
            "personality_prompt": {"role": "ParentB"},
            "enabled": True,
        },
    ]
...
async def test_combine_personalities_success(mock_active_personalities):
    ...
    with (
        patch(
            "backend.evolution_service.get_active_personalities",
            return_value=mock_active_personalities,
        ),
        patch(
            "backend.evolution_service.generate_feedback_summary",
            side_effect=["Feedback A", "Feedback B"],
        ),
        patch(
            "backend.evolution_service.query_model", return_value={"content": llm_response_content}
        ),
        patch(
            "backend.config.personalities.load_org_system_prompts",
            return_value={"evolution_prompt": "Prompt: {parent_data}"},
        ),
        patch(
            "backend.config.personalities.load_org_models_config",
            return_value={"chairman_model": "gpt-4"},
        ),
...
async def test_combine_personalities_not_enough_parents(mock_active_personalities):
    ...
    with patch(
        "backend.evolution_service.get_active_personalities", return_value=mock_active_personalities
    ):
...
async def test_combine_personalities_invalid_yaml(mock_active_personalities):
    ...
    with (
        ...
        patch(
            "backend.config.personalities.load_org_system_prompts",
            return_value={"evolution_prompt": "Prompt"},
        ),
        patch(
            "backend.config.personalities.load_org_models_config",
            return_value={"chairman_model": "gpt-4"},
