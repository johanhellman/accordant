...
def test_rate_limiting_chat(client, enable_rate_limiter, system_db_session):
    ...
    with patch("backend.storage.get_conversation") as mock_get_conv:
        ...
        with patch("backend.main._handle_conversation_title", new_callable=AsyncMock), \
            ...
            # Hit chat 5 times
            for _ in range(5):
                res = client.post("/api/conversations/c1/message", json={"content": "hi"}, headers={"X-Forwarded-For": "10.0.0.1"})
