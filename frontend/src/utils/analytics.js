...
export function initAnalytics() {
  ...
  const analyticsDomain =
    import.meta.env.VITE_ANALYTICS_DOMAIN || import.meta.env.VITE_PLAUSIBLE_DOMAIN;
...
function initUmami(domain, websiteId) {
  ...
  script.onerror = () => {
    ...
    if (import.meta.env.DEV) {
      console.warn(
        `[Analytics] Failed to load Umami script from ${scriptUrl}. Check DNS or VITE_ANALYTICS_DOMAIN.`
      );
    }
...
function initPlausible(domain) {
  ...
  if (import.meta.env.DEV) {
    console.log(
      `[Analytics] Initialized Plausible - Domain: ${analyticsDomain}, Script: ${scriptUrl}`
    );
  }
}
