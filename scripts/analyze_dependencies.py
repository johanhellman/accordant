...
def get_latest_python_version(package_name: str) -> str | None:
...
def compare_versions(current: str, latest: str | None) -> tuple[str, str]:
    """
            ...
            if cmp_result < 0:
                ...
                if len(latest_parts) > 0 and len(current_parts) > 0:
                    if int(latest_parts[0]) > int(current_parts[0]):
                        return (
                            "major_update",
                            f"Major update available: {current_version} → {latest}",
                        )
                    elif len(latest_parts) > 1 and len(current_parts) > 1:
                        if int(latest_parts[1]) > int(current_parts[1]):
                            return (
                                "minor_update",
                                f"Minor update available: {current_version} → {latest}",
                            )
                        else:
                            return (
                                "patch_update",
                                f"Patch update available: {current_version} → {latest}",
                            )
...
def print_report(python_results: list[dict], js_results: list[dict]):
    ...
    print("=" * 80)
    print(
        f"Python: {len(major_updates)} major, {len(minor_updates)} minor, {len(patch_updates)} patch updates"
    )
    print(
        f"JavaScript: {len(major_updates_js)} major, {len(minor_updates_js)} minor, {len(patch_updates_js)} patch updates"
    )
