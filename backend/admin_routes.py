...
from .organizations import delete_org, get_org, get_org_api_config, list_orgs, update_org
from .ranking_service import (
    calculate_instance_league_table,
    calculate_league_table,
    generate_feedback_summary,
)
from .security import encrypt_value
from .users import User, get_all_users
...
async def get_system_prompts(current_user: User = Depends(get_current_admin_user)):
    ...
    from .config.personalities import load_org_models_config, load_org_system_prompts_config
    ...
    return {
        ...
        "ranking_enforced_format": ENFORCED_OUTPUT_FORMAT.replace(
            "{FINAL_RANKING_MARKER}", "FINAL RANKING:"
        ).replace("{RESPONSE_LABEL_PREFIX}", "Response "),
        "chairman": {
...
async def get_default_system_prompts(current_user: User = Depends(get_current_instance_admin)):
    ...
    chairman_prompt = (
        defaults.get("chairman", {}).get("prompt", "")
        if isinstance(defaults.get("chairman"), dict)
        else ""
    )
    title_prompt = (
        defaults.get("title_generation", {}).get("prompt", "")
        if isinstance(defaults.get("title_generation"), dict)
        else ""
    )
...
async def update_default_system_prompts(
    ...
    if config.get("title_generation") and isinstance(config["title_generation"], dict):
        if "title_generation" not in current_defaults or not isinstance(
            current_defaults["title_generation"], dict
        ):
            current_defaults["title_generation"] = {}
        current_defaults["title_generation"]["prompt"] = get_val(
            config["title_generation"].get("prompt")
        )
...
async def update_system_prompts(config: dict, current_user: User = Depends(get_current_admin_user)):
    """
    ...
    """
    from .config.personalities import get_org_config_dir
    ...
    update_field(
        current_config, "stage1_response_structure", config.get("stage1_response_structure")
    )
    ...
    # Title Gen
    if "title_generation" not in current_config:
        current_config["title_generation"] = {}
...
async def update_org_settings(
    ...
    if not current_user.org_id:
        raise HTTPException(
            status_code=400, detail="User must belong to an organization to update settings"
        )
...
async def get_personality_feedback(
    ...
    summary = await generate_feedback_summary(current_user.org_id, p["name"], api_key, base_url)
...
async def combine_personalities_route(
    ...
    try:
        ...
        new_personality = await combine_personalities(
            current_user.org_id, request.parent_ids, request.name_suggestion, api_key, base_url
        )
