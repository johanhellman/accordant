...
def format_personality_prompt(
    personality: dict[str, Any], system_prompts: dict[str, str], include_enforced: bool = True
) -> str:
    """
...
def load_org_system_prompts_config(org_id: str) -> dict[str, dict[str, Any]]:
    ...
    def _create_entry(
        key: str, default_val: str, org_val: str | None, section: str = None
    ) -> dict[str, Any]:
        is_custom = False
        value = default_val
        ...
        if section:
            ...
            if (
                section in org_config
                and isinstance(org_config[section], dict)
                and key in org_config[section]
            ):
    ...
    result["title_prompt"] = _create_entry(
        "prompt", default_title, None, section="title_generation"
    )
    result["evolution_prompt"] = _create_entry("evolution_prompt", default_evolution, None)
    result["stage1_response_structure"] = _create_entry(
        "stage1_response_structure", default_struct_resp, None
    )
    result["stage1_meta_structure"] = _create_entry(
        "stage1_meta_structure", default_struct_meta, None
    )

    # New migrated prompts
    result["ranking_enforced_context"] = _create_entry(
        "ranking_enforced_context", default_ranking_enforced_context, None
    )
    result["ranking_enforced_format"] = _create_entry(
        "ranking_enforced_output_format", default_ranking_enforced_format, None
    )
    result["feedback_synthesis_prompt"] = _create_entry(
        "feedback_synthesis_prompt", default_feedback_synthesis, None
    )
    ...
    if "ranking_prompt" in org_config:
        ...
        "ranking" in org_config
        and isinstance(org_config["ranking"], dict)
        and "prompt" in org_config["ranking"]
    ):
...
def load_org_models_config(org_id: str) -> dict[str, str]:
    ...
    default_chairman_model = _get_nested_config_value(
        defaults, "chairman", "model", "gemini/gemini-2.5-pro"
    )
    default_title_model = _get_nested_config_value(
        defaults, "title_generation", "model", "gemini/gemini-2.5-pro"
    )
...
def get_all_personalities(org_id: str) -> list[dict[str, Any]]:
    ...
    _load_org_config_file(org_id) or {}
